<?php
	...
	class wordSplit{
		...
		function __construct(){
			...
			$this->db = new db();
			$this->db->connect() or die(__FILE__.'数据库连接出错');
			
			mb_internal_encoding('utf8');  //设置/获取内部字符编码
		...
		static function getDic(){
			...
			if (file_exists(self::$dicfile)) {
				...
				while(!feof(self::$fp)){  //函数检测是否已到达文件末尾
					$line = fgets(self::$fp);	  // 函数从文件指针中读取一行
					$splitword = explode('	', $line);	//函数根据‘’字符串分割为数组
						...
						for($j=$n-1;$j>$i;$j--){
							...
							if($this->isfind($word) || in_array($word,$spec_words) || preg_match('/^[A-Za-z0-9\-\.\/]+$/',$word)){
								/*if(strlen($word)>3){//检查是否能够再分词
									...
									$n2 = strlen($word);
									for($i2=0;$i2<$n2;$i2++){
										$w1 = mb_substr($word,0,$i2+1,'utf8');
										$w2 = mb_substr($word,$i2+1,$n2-$i2-1,'utf8');
										//echo "$i2#$n2,";
			...
			foreach($sentence as $sen){
				...
				if($sen != ''){
					...
					while($i < $j){
						...
						$isfind = false;
						for(;$j>$i;$i++){
							$word = mb_substr($sen,$i,$j-$i,'utf8');   //word每一串获取的字
							//echo "[$i#$j,".($word)."]<br/>";
							$can_split = false;
							$search_word = $this->getWords($word);	 //356
							
							if(!empty($search_word)){	
								//print_r($search_word);
								$this->realtSplitWords .= $word."|"; //第一串字
					...
					if(count($splitword)){
						$words[$key]=array_reverse($splitword);	 //返回一个单元顺序相反的数组
						$key++;
					}
		...
		function getRealtSplitWords(){
			...
			foreach($sen as $s){
				if($s != ''){
					$words = explode('|',$s);
					$words = implode('|',array_reverse($words));//用'|'将一维数组的值连接为一个字符串
					$r_sen .= $words.'#';
				}
			}
			return $r_sen;
		...
		function isfind($word){
			...
			return $this->db->getNum("words",$where);
		}
		
		function getWords($word){
			$where = array('word'=>$word);
			$getword = $this->db->fetchRow("words",$where);
		...
		function getNegWords($word){
			...
			if(!preg_match('/^\s+$/',$word) && $word !=''){
				$sql = "SELECT * FROM `negative` WHERE `word1`='$word' OR `word2`='$word' OR `word3`='$word' OR `word4`='$word' OR `word5`='$word'";
		...
		function isChinesePercent($word){
			if(strpos($word,'百分之') == 0){	 //查找字符串首次出现的位置
				...
				for($k=0;$k<$len;$k++){
					$subword = mb_substr($c_word,$k,1);//获取字符串的部分 逐个获取
